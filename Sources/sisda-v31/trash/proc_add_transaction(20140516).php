...
if(isset($_SESSION["id"]) && $_SESSION["privilege"] == "2") {
	...
	if(!empty($_POST["subtotal_daful"]) && $_POST["subtotal_daful"] != 0) {
		...
		$kegiatan_daful						= empty($_POST["kegiatan_daful"]) ? 0 : $_POST["kegiatan_daful"];
		$kegiatan_daful_enc					= mysql_real_escape_string($kegiatan_daful);
		
		$peralatan_daful					= empty($_POST["peralatan_daful"]) ? 0 : $_POST["peralatan_daful"];
		$peralatan_daful_enc				= mysql_real_escape_string($peralatan_daful);
		
		$seragam_daful						= empty($_POST["seragam_daful"]) ? 0 : $_POST["seragam_daful"];
		$seragam_daful_enc					= mysql_real_escape_string($seragam_daful);
		
		$luncil_daful						= empty($_POST["luncil_daful"]) ? 0 : $_POST["luncil_daful"];
		$luncil_daful_enc					= mysql_real_escape_string($luncil_daful);
		...
		$src_get_tunggakan_daful	= "select nominal_tunggakan from tunggakan where no_sisda = '$no_sisda_enc' and periode = '$tahun_daftar_ulang_enc' and jenis_tunggakan = 'daftar_ulang'";
		...
		$current_errear_daful	= $last_arrear_daful - $subtotal_daful_enc;
		
		$src_update_tunggakan_daful		= "update tunggakan set nominal_tunggakan = '$current_errear_daful' where  no_sisda = '$no_sisda_enc' and periode = '$tahun_daftar_ulang_enc' and jenis_tunggakan = 'daftar_ulang'";
	...
	if(!empty($_POST["subtotal_bima"]) && $_POST["subtotal_bima"] != 0) {
		...
		$src_get_tunggakan_bima		= "select nominal_tunggakan from tunggakan where no_sisda = '$no_sisda_enc' and jenis_tunggakan = 'biaya_masuk'";
		...
		$current_arrear_bima 	= $last_arrear_bima - $subtotal_bima_enc;
		
		$src_update_tunggakan_bima		= "update tunggakan set nominal_tunggakan = '$current_arrear_bima' where  no_sisda = '$no_sisda_enc' and jenis_tunggakan = 'biaya_masuk'";
		...
		if($query_update_tunggakan_bima) {
			...
			$src_input_bima	= "insert into transaksi (
									...
									'$tahap_bima_enc',
									'$lucil_bima_enc',
									'$tanggal_bayar_berikut_bima_enc',
									'bima' 
									$if_transfer_value
									)";
