    	...
    	<td align="center">
			...
			if($query_clear_buf_tung) {
				...
				while($row_no_sisda_distc = mysql_fetch_array($query_no_sisda_disct)) {
					...
					$src_no_sisda_chk	= $no_sisda_chk['no_sisda'];
					
					
					$src_grab_tunggakan		= "select * from tunggakan left join siswa on siswa.no_sisda = tunggakan.no_sisda where no_sisda = '$src_no_sisda_chk'"; 
					$query_gab_tunggakan	= mysqli_query($mysql_connect, $src_grab_tunggakan) or die(mysql_error());
					
					while($query_gab_tunggakan = mysql_fetch_array) {
						
						$src_buf_nama_siswa	= $query_gab_tunggakan['nama_siswa'];
						$src_buf_name 		= $query_gab_tunggakan["jenis_tunggakan"];
						$src_buf_kelas 		= $query_gab_tunggakan["kelas"];
						$src_buf_tingkat 	= $query_gab_tunggakan["tingkat"];
						$src_buf_periode	= $query_gab_tunggakan['periode'];
						
						if($query_gab_tunggakan["jenis_tunggakan"] = "biaya_masuk") {
						
							$src_buf_nom_biaya_masuk = $query_gab_tunggakan["nominal_tunggakan"];
						
						} else if($query_gab_tunggakan["jenis_tunggakan"] = "daftar_ulang") {
						
							$src_buf_nom_daftar_ulang = $query_gab_tunggakan["nominal_tunggakan"];
						
						} else if($query_gab_tunggakan["jenis_tunggakan"] = "spp") {
						
							$src_buf_tung_july		= $query_gab_tunggakan["july"];	$src_sts_tung_july = substr($src_buf_tung_july,0,1); $src_nom_tung_july = substr($src_buf_tung_july,2);
							$src_buf_tung_august 	= $query_gab_tunggakan["august"]; $src_sts_tung_august = substr($src_buf_tung_august,0,1); $src_nom_tung_august = substr($src_buf_tung_august,2);
							$src_buf_tung_september	= $query_gab_tunggakan["september"]; $src_sts_tung_september = substr($src_buf_tung_september,0,1); $src_nom_tung_september = substr($src_buf_tung_september,2);
							$src_buf_tung_october 	= $query_gab_tunggakan["october"]; $src_sts_tung_october = substr($src_buf_tung_october,0,1); $src_nom_tung_october = substr($src_buf_tung_october,2);
							$src_buf_tung_november 	= $query_gab_tunggakan["november"]; $src_sts_tung_november = substr($src_buf_tung_november,0,1); $src_nom_tung_november = substr($src_buf_tung_november,2);
							$src_buf_tung_december	= $query_gab_tunggakan["december"]; $src_sts_tung_december = substr($src_buf_tung_december,0,1); $src_nom_tung_december = substr($src_buf_tung_december,2);
							$src_buf_tung_january 	= $query_gab_tunggakan["january"]; $src_sts_tung_january = substr($src_buf_tung_january,0,1); $src_nom_tung_january = substr($src_buf_tung_january,2);
							$src_buf_tung_february 	= $query_gab_tunggakan["february"]; $src_sts_tung_february = substr($src_buf_tung_february,0,1); $src_nom_tung_february = substr($src_buf_tung_february,2);
							$src_buf_tung_march 	= $query_gab_tunggakan["march"]; $src_sts_tung_march = substr($src_buf_tung_march,0,1); $src_nom_tung_march = substr($src_buf_tung_march,2);
							$src_buf_tung_april		= $query_gab_tunggakan["april"]; $src_sts_tung_april = substr($src_buf_tung_april,0,1); $src_nom_tung_april = substr($src_buf_tung_april,2);
							$src_buf_tung_may 		= $query_gab_tunggakan["may"]; $src_sts_tung_may = substr($src_buf_tung_may,0,1); $src_nom_tung_may = substr($src_buf_tung_may,2);
							$src_buf_tung_june		= $query_gab_tunggakan["june"];	$src_sts_tung_june = substr($src_buf_tung_june,0,1); $src_nom_tung_june = substr($src_buf_tung_june,2);
						...
						} else if($query_gab_tunggakan["jenis_tunggakan"] = "catering") {
						
							$src_buf_tung_july_catering			= $query_gab_tunggakan["jul_cataj"]; $src_sts_tung_july_catering = substr($src_buf_tung_july_catering,0,1); $src_nom_tung_july_catering = substr($src_buf_tung_july_catering,2);
							$src_buf_tung_august_catering 		= $query_gab_tunggakan["aug_cataj"]; $src_sts_tung_august_catering = substr($src_buf_tung_august_catering,0,1); $src_nom_tung_august_catering = substr($src_buf_tung_august_catering,2);
							$src_buf_tung_september_catering	= $query_gab_tunggakan["sep_cataj"]; $src_sts_tung_september_catering = substr($src_buf_tung_september_catering,0,1); $src_nom_tung_september_catering = substr($src_buf_tung_september_catering,2);
							$src_buf_tung_october_catering 		= $query_gab_tunggakan["oct_cataj"]; $src_sts_tung_october_catering = substr($src_buf_tung_october_catering,0,1); $src_nom_tung_october_catering = substr($src_buf_tung_october_catering,2);
							$src_buf_tung_november_catering 	= $query_gab_tunggakan["nov_cataj"]; $src_sts_tung_november_catering = substr($src_buf_tung_november_catering,0,1); $src_nom_tung_november_catering = substr($src_buf_tung_november_catering,2);
							$src_buf_tung_december_catering		= $query_gab_tunggakan["dec_cataj"]; $src_sts_tung_december_catering = substr($src_buf_tung_december_catering,0,1); $src_nom_tung_december_catering = substr($src_buf_tung_december_catering,2);
							$src_buf_tung_january_catering 		= $query_gab_tunggakan["jan_cataj"]; $src_sts_tung_january_catering = substr($src_buf_tung_january_catering,0,1); $src_nom_tung_january_catering = substr($src_buf_tung_january_catering,2);
							$src_buf_tung_february_catering 	= $query_gab_tunggakan["feb_cataj"]; $src_sts_tung_february_catering = substr($src_buf_tung_february_catering,0,1); $src_nom_tung_february_catering = substr($src_buf_tung_february_catering,2);
							$src_buf_tung_march_catering 		= $query_gab_tunggakan["mar_cataj"]; $src_sts_tung_march_catering = substr($src_buf_tung_march_catering,0,1); $src_nom_tung_march_catering = substr($src_buf_tung_march_catering,2);
							$src_buf_tung_april_catering		= $query_gab_tunggakan["apr_cataj"]; $src_sts_tung_april_catering = substr($src_buf_tung_april_catering,0,1); $src_nom_tung_april_catering = substr($src_buf_tung_april_catering,2);
							$src_buf_tung_may_catering 			= $query_gab_tunggakan["may_cataj"]; $src_sts_tung_may_catering = substr($src_buf_tung_may_catering,0,1); $src_nom_tung_may_catering = substr($src_buf_tung_may_catering,2);
							$src_buf_tung_june_catering			= $query_gab_tunggakan["jun_cataj"]; $src_sts_tung_june_catering = substr($src_buf_tung_june_catering,0,1); $src_nom_tung_june_catering = substr($src_buf_tung_june_catering,2);
						...
						} else if($query_gab_tunggakan["jenis_tunggakan"] = "antar_jemput") {
						
							$src_buf_tung_july_antar_jemput			= $query_gab_tunggakan["jul_cataj"]; $src_sts_tung_july_antar_jemput = substr($src_buf_tung_july_antar_jemput,0,1); $src_nom_tung_july_antar_jemput = substr($src_buf_tung_july_antar_jemput,2);
							$src_buf_tung_august_antar_jemput 		= $query_gab_tunggakan["aug_cataj"]; $src_sts_tung_august_antar_jemput = substr($src_buf_tung_august_antar_jemput,0,1); $src_nom_tung_august_antar_jemput = substr($src_buf_tung_august_antar_jemput,2);
							$src_buf_tung_september_antar_jemput	= $query_gab_tunggakan["sep_cataj"]; $src_sts_tung_september_antar_jemput = substr($src_buf_tung_september_antar_jemput,0,1); $src_nom_tung_september_antar_jemput = substr($src_buf_tung_september_antar_jemput,2);
							$src_buf_tung_october_antar_jemput 		= $query_gab_tunggakan["oct_cataj"]; $src_sts_tung_october_antar_jemput = substr($src_buf_tung_october_antar_jemput,0,1); $src_nom_tung_october_antar_jemput = substr($src_buf_tung_october_antar_jemput,2);
							$src_buf_tung_november_antar_jemput 	= $query_gab_tunggakan["nov_cataj"]; $src_sts_tung_november_antar_jemput = substr($src_buf_tung_november_antar_jemput,0,1); $src_nom_tung_november_antar_jemput = substr($src_buf_tung_november_antar_jemput,2);
							$src_buf_tung_december_antar_jemput		= $query_gab_tunggakan["dec_cataj"]; $src_sts_tung_december_antar_jemput = substr($src_buf_tung_december_antar_jemput,0,1); $src_nom_tung_december_antar_jemput = substr($src_buf_tung_december_antar_jemput,2);
							$src_buf_tung_january_antar_jemput 		= $query_gab_tunggakan["jan_cataj"]; $src_sts_tung_january_antar_jemput = substr($src_buf_tung_january_antar_jemput,0,1); $src_nom_tung_january_antar_jemput = substr($src_buf_tung_january_antar_jemput,2);
							$src_buf_tung_february_antar_jemput 	= $query_gab_tunggakan["feb_cataj"]; $src_sts_tung_february_antar_jemput = substr($src_buf_tung_february_antar_jemput,0,1); $src_nom_tung_february_antar_jemput = substr($src_buf_tung_february_antar_jemput,2);
							$src_buf_tung_march_antar_jemput 		= $query_gab_tunggakan["mar_cataj"]; $src_sts_tung_march_antar_jemput = substr($src_buf_tung_march_antar_jemput,0,1); $src_nom_tung_march_antar_jemput = substr($src_buf_tung_march_antar_jemput,2);
							$src_buf_tung_april_antar_jemput		= $query_gab_tunggakan["apr_cataj"]; $src_sts_tung_april_antar_jemput = substr($src_buf_tung_april_antar_jemput,0,1); $src_nom_tung_april_antar_jemput = substr($src_buf_tung_april_antar_jemput,2);
							$src_buf_tung_may_antar_jemput 			= $query_gab_tunggakan["may_cataj"]; $src_sts_tung_may_antar_jemput = substr($src_buf_tung_may_antar_jemput,0,1); $src_nom_tung_may_antar_jemput = substr($src_buf_tung_may_antar_jemput,2);
							$src_buf_tung_june_antar_jemput			= $query_gab_tunggakan["jun_cataj"]; $src_sts_tung_june_antar_jemput = substr($src_buf_tung_june_antar_jemput,0,1); $src_nom_tung_june_antar_jemput = substr($src_buf_tung_june_antar_jemput,2);
