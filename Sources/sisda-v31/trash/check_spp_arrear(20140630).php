...
if($cur_month == "july") {
	...
	if(mysql_num_rows($query_last_july) != 0) {
		...
		while($row_last_july = mysql_fetch_array($query_last_july)) {
			...
			if($row_last_july["jenis_tunggakan"] == "spp") {
				...
				if(substr($row_last_july["july"],0,1) == 1 || substr($row_last_july["july"],0,1) == 3) {
				...
				if(substr($row_last_july["jul_cataj"],0,1) == 1) {
				
					$no_sisda_last_july 		= $row_last_july["no_sisda"];					
					$exp_cataj_last_july 		= explode("-",$row_last_july["jul_cataj"]);
					//bisa catering & bisa antar jemput
	...
	if(mysql_num_rows($query_cur_non_july) != 0 ) {
