...
if(isset($_SESSION["id"]) && $_SESSION["privilege"] == "2") {
	...
	if($no_sisda != "" && $periode != "" && ($sis_fin != "" || $tunggakan != "")) {
		...
		$src_chk_sisfin_cur 	= "select id from siswa_finance where no_sisda = '$no_sisda_enc' and periode = '$periode_enc'";
		...
		if($num_chk_sisfin_cur == "0") {
			...
			if($tingkat_lastyear == "toddler")	{ $tingkat_cur = "PG";		$jenjang_cur = "PG"; }
			if($tingkat_lastyear == "PG") 		{ $tingkat_cur = "TK A"; 	$jenjang_cur = "TK A"; }
			if($tingkat_lastyear == "TK A") 	{ $tingkat_cur = "TK B";	$jenjang_cur = "TK B"; }
			if($tingkat_lastyear == "TK B") 	{ $tingkat_cur = "1";		$jenjang_cur = "SD"; }
			if($tingkat_lastyear == 1)			{ $tingkat_cur = "2";		$jenjang_cur = "SD"; }
			if($tingkat_lastyear == 2) 			{ $tingkat_cur = "3";		$jenjang_cur = "SD"; }
			if($tingkat_lastyear == 3) 			{ $tingkat_cur = "4";		$jenjang_cur = "SD"; }
			if($tingkat_lastyear == 4) 			{ $tingkat_cur = "5";		$jenjang_cur = "SD"; }
			if($tingkat_lastyear == 5) 			{ $tingkat_cur = "6";		$jenjang_cur = "SD"; }
			if($tingkat_lastyear == 6) 			{ $tingkat_cur = "7";		$jenjang_cur = "SMP"; }
			if($tingkat_lastyear == 7) 			{ $tingkat_cur = "8";		$jenjang_cur = "SMP"; }
			if($tingkat_lastyear == 8) 			{ $tingkat_cur = "9";		$jenjang_cur = "SMP"; }
			
			//echo "<h1>tinglast=".$tingkat_lastyear."</h1>";
			
			$src_get_spp_cur	= "select nominal,item_byr from set_spp where tingkat = '$tingkat_cur' and periode = '$periode_enc' and ket_disc = '$tahun_siswa_masuk'";
			...
			if($num_get_spp_cur != 0) {
				...
				$all_spp_cur = $cur_nom_spp + $cur_nom_ict + $cur_nom_kts + $cur_nom_ler + $cur_nom_kts;
				...
				if($query_add_new_data_finance) {
					...
					if($query_change_last_status) { $go_check_tunggakan = "ya"; }
				...
				echo "
						...
						Data keuangan siswa <b>".ucwords($src_nama_siswa)."</b> untuk tahun ajaran <b>$periode_enc</b> tidak dapat dibuat<br>
