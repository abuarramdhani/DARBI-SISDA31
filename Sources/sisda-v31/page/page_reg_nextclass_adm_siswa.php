    ...
    <tr>
    	...
    	<td>
            ...
            if($num_get_data_siswa_finance >> 0) {
				...
				//we need this query for taking data parents only, the rest data will be completed by query finance below
				...
				//this is the query finance
				//Why aktif = 1, because when aktif = 1, it means that is the last(current) data being used by siswa
				//that data shows where 'tingkat' that the siswa stay in
				$src_get_data_finance	= "select * from siswa_finance where no_sisda = '$existing_no_sisda' and aktif = '1'";
				$query_get_data_finance	= mysqli_query($mysql_connect, $src_get_data_finance) or die("There is an error with mysql: ".mysql_error());
				$row_get_data_finance	= mysql_fetch_array($query_get_data_finance);
			?>
        	...
        	<table width="100%" border="0" cellpadding="0" cellspacing="0">
				...
				*/
                ...
                <tr height="20">
                    ...
                    <td width="5"></td>
                    <td>
                    <?PHP //This field using $row_get_data_finance ?>
                    <select name="tingkat">
                    <option value="">Pilih</option>
                    <option value="Toddler" <?PHP if($row_get_data_finance["tingkat"]=="Toddler") { ?>selected="selected"<?PHP } ?>>Toddler</option>
                    <option value="PG"      <?PHP if($row_get_data_finance["tingkat"]=="PG") { ?>selected="selected"<?PHP } ?>>PG</option>
                    <option value="TK A"	<?PHP if($row_get_data_finance["tingkat"]=="TK A") { ?>selected="selected"<?PHP } ?>>TK A</option>
                    <option value="TK B"	<?PHP if($row_get_data_finance["tingkat"]=="TK B") { ?>selected="selected"<?PHP } ?>>TK B</option>
                    <option value="1"		<?PHP if($row_get_data_finance["tingkat"]=="1") { ?>selected="selected"<?PHP } ?>>1</option>
                    <option value="2"		<?PHP if($row_get_data_finance["tingkat"]=="2") { ?>selected="selected"<?PHP } ?>>2</option>
                    <option value="3"		<?PHP if($row_get_data_finance["tingkat"]=="3") { ?>selected="selected"<?PHP } ?>>3</option>
                    <option value="4"		<?PHP if($row_get_data_finance["tingkat"]=="4") { ?>selected="selected"<?PHP } ?>>4</option>
                    <option value="5"		<?PHP if($row_get_data_finance["tingkat"]=="5") { ?>selected="selected"<?PHP } ?>>5</option>
                    <option value="6"		<?PHP if($row_get_data_finance["tingkat"]=="6") { ?>selected="selected"<?PHP } ?>>6</option>
                    <option value="7"		<?PHP if($row_get_data_finance["tingkat"]=="7") { ?>selected="selected"<?PHP } ?>>7</option>
                    <option value="8"		<?PHP if($row_get_data_finance["tingkat"]=="8") { ?>selected="selected"<?PHP } ?>>8</option>
                    <option value="9"		<?PHP if($row_get_data_finance["tingkat"]=="9") { ?>selected="selected"<?PHP } ?>>9</option>
                    </select>
                    <?PHP
					/*
