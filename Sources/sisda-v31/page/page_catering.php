...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	...
	<tr height="25">
    	<td width="30"></td>
        <td id="text_title_page1" align="center">Catering siswa</td>
        <td width="30"></td>
    </tr>
    <tr>
    	<td></td>
    ...
    <?PHP
	$src_check_penyedia_catering = "select id from cataj where type = 'Catering'";
	$query_check_penyedia_catering = mysqli_query($mysql_connect, $src_check_penyedia_catering) or die(mysql_error());
	
	if(mysql_num_rows($query_check_penyedia_catering) == 0) {
	?>
    	<tr>
        	<td height="40">&nbsp;</td>
        </tr>
    	<tr>
            ...
            Data penyedia layanan catering belum diinput. Proses pendaftaran catering siswa tidak dapat dilakukan. <br />Silakan lengkapi data penyedia catering pada halaman 'Biaya catering 'terlebih dahulu. <br /><br /><input type="button" value="Biaya catering" onclick="window.location.href='?pl=cataj';" style="height:40px; width:150px;" /></td>
            <td></td>
        </tr>
	<?PHP
	} else {
    ...
    <tr>
    	...
    	<td align="center">
            ...
            <form name="search" method="post" action="?pl=catering">
            	...
            	<tr height="20">
					...
					for($i = 1; $i <= $all_page; $i++) {
						if($i == $p) {
							...
							echo "<span id='paging'><a href=\"?pl=catering&p=$i&$v_show&s=$srch_no_sisda&j=$srch_jenjang&n=$srch_nama_siswa&periode=$srch_periode\" >$i</a></span> ";
						}
					}
					?>
                    </td>
			...
			<form method="post" name="add_del_catering" action="engine.php?case=add_del_catering">
            <?PHP
            ...
            <?PHP
			$get_catering		= "select * from cataj where type = 'Catering'";
			$query_catering		= mysqli_query($mysql_connect, $get_catering) or die ("terjadi kesalahan: ".mysql_error());	
			$query_catering2	= mysqli_query($mysql_connect, $get_catering) or die ("terjadi kesalahan: ".mysql_error());	
			?>
            <table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#333333">
            	...
            	<tr>
                    ...
                    <select name="catering_name" style="width:195px; height:40px; font-size:14px;">
                    <option value="">Penyedia layanan catering</option>
                    <?PHP
					while($row_catering	= mysql_fetch_array($query_catering)) {
					?>
					<option value="<?= $row_catering["name"]; ?>"><?= $row_catering["name"]; ?> [Rp <?= $row_catering["nominal"]; ?>]</option>
					<?PHP
					}
					?>
                    </select>
                    ...
                    <input type="submit" name="daftarkan" value="Daftarkan Catering" style="width:150px; height:40px; font-size:14px; color:#FFFFFF; background-color:#336699;" onClick="return verification()" />
                    </td>
                    <td>&nbsp;</td>
                    <td width="150" align="right">
                    <input type="submit" name="berhenti" value="Berhenti Catering" style="width:150px; height:40px; font-size:14px; color:#FFFFFF; background-color:#ff0000;" onClick="return verification1()" />
                    </td>
                    <td width="20">&nbsp;</td>
                </tr>
                <tr>
        	...
        	<table width="100%" border="0" cellpadding="0" cellspacing="0">            	             
                ...
                <tr height="30">                	
                    ...
                    <td width="10" bgcolor="<?PHP echo $bg; ?>"></td>
                    <td width="" bgcolor="<?PHP echo $bg; ?>" id="text_normal_black" align="center"><?PHP if($get_siswa["catering"] != "") { echo $get_siswa["catering"]; } else { echo "Tidak catering"; } ?></td>
                    <td width="10" bgcolor="<?PHP echo $bg; ?>"></td>
...
function verification() { 
	...
	if(document.add_del_catering.catering_name.value == "") {
		alert('Field "Penyedia layanan catering" tidak boleh kosong');
		return false;
	}
	if(document.add_del_catering.day_catering.value == "") {
