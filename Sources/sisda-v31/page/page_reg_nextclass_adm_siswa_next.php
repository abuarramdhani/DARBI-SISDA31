...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr>
    	...
    	<td>
			...
			<form method="post" name="reg_adm_siswa" action="page/page_reg_nextclass_adm_siswa_redirect.php">
            ...
            <?PHP //we have to change variable 'periode' to 'ori_periode', because, because we have to keep it ?>
            <input type="hidden" name="send_periode" value="<?PHP $_POST["send_periode"]; ?>" />             
        	<table width="100%" border="0" cellpadding="0" cellspacing="0">
            	<tr height="45">
				...
				if($_POST["tingkat"] == "Toddler") 	{ $jenjang = "Toddler"; }
				if($_POST["tingkat"] == "PG") 		{ $jenjang = "PG"; }
				if($_POST["tingkat"] == "TK A") 	{ $jenjang = "TK A"; }
				if($_POST["tingkat"] == "TK B") 	{ $jenjang = "TK B"; }
				if($_POST["tingkat"] == "1") 		{ $jenjang = "SD"; }
				if($_POST["tingkat"] == "2") 		{ $jenjang = "SD"; }
				if($_POST["tingkat"] == "3") 		{ $jenjang = "SD"; }
				if($_POST["tingkat"] == "4") 		{ $jenjang = "SD"; }
				if($_POST["tingkat"] == "5") 		{ $jenjang = "SD"; }
				if($_POST["tingkat"] == "6") 		{ $jenjang = "SD"; }
				if($_POST["tingkat"] == "7") 		{ $jenjang = "SMP"; }
				if($_POST["tingkat"] == "8") 		{ $jenjang = "SMP"; }
				if($_POST["tingkat"] == "9") 		{ $jenjang = "SMP"; }
				?>       
                <tr height="20">
                	<td width="10" bgcolor="#999999"></td>
                ...
                <tr height="20">
                    ...
                    <td width="5"></td>
                    <td><input type="text" name="tingkat" size="35"  value="<?PHP echo $_POST["tingkat"]; ?>" readonly="readonly"/></td>
                </tr>
                <tr>
                	<td colspan="4" height="5"></td>
                </tr>
            ...
            <table border="0" cellpadding="0" cellspacing="0">            	
                <tr height="20">
                  ...
                  <td width="200" bgcolor="#333333" id="text_normal_white">Kategori administrasi</td>
                    ...
                    <td>
                    <?PHP
					//Please this condition carefully.
					//Re-registration is implemanted on TK A (from PG) and TK B (from TK B)
					//So, we have to make other 'tingkat' (outside TK A & TK B) automatically display disc_cat_adm defined at thi first registration
					
					if($jenjang == "TK A" || $jenjang == "TK B") {
						...
						<option value="1" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "1") { ?> selected="selected"<?PHP $q = "umum"; }} ?>>Umum</option>
						<option value="2" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "2") { ?> selected="selected"<?PHP $q = "berdesaukan"; }} ?>>Bersamaan dengan saudara kandung</option>
						<option value="3" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "3") { ?> selected="selected"<?PHP $q = "memsaukan"; }} ?>>Memiliki saudara kandung</option>
						<option value="4" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "4") { ?> selected="selected"<?PHP $q = "umgrab"; }} ?>>Umum grade B</option>
						<option value="5" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "5") { ?> selected="selected"<?PHP $q = "umskgrab"; }} ?>>Umum memiliki Saudara Kandung + Grade B</option>
						<option value="6" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "6") { ?> selected="selected"<?PHP $q = "ubdskgrab"; }} ?>>Umum bersamaan dengan Saudara Kandung + Grade B</option>
						<option value="7" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "7") { ?> selected="selected"<?PHP $q = "asdar"; }} ?>>Asal Darbi</option>
						<option value="8" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "8") { ?> selected="selected"<?PHP $q = "asdargraa"; }} ?>>Asal Darbi + Grade A</option>
						<option value="9" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "9") { ?> selected="selected"<?PHP $q = "asdargrab"; }} ?>>Asal Darbi + Grade B</option>
						<option value="10" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "10") { ?> selected="selected"<?PHP $q = "anpeg1"; }} ?>>Anak pegawai ke-1</option>
						<option value="11" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "11") { ?> selected="selected"<?PHP $q = "anpeg2"; }} ?>>Anak pegawai ke-2</option>
						<option value="12" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "12") { ?> selected="selected"<?PHP $q = "anpeg3"; }} ?>>Anak pegawai ke-3, dst</option>
						<option value="13" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "13") { ?> selected="selected"<?PHP $q = "anpeg1graa"; }} ?>>Anak pegawai ke-1 + Grade A</option>
						<option value="14" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "14") { ?> selected="selected"<?PHP $q = "anpeg1grab"; }} ?>>Anak pegawai ke-1 + Grade B</option>
						<option value="15" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "15") { ?> selected="selected"<?PHP $q = "anpeg2graa"; }} ?>>Anak pegawai ke-2 + Grade A</option>
						<option value="16" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "16") { ?> selected="selected"<?PHP $q = "anpeg2grab"; }} ?>>Anak pegawai ke-2 + Grade B</option>
						<option value="17" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "17") { ?> selected="selected"<?PHP $q = "anpeg3graa"; }} ?>>Anak pegawai ke-3, dst + Grade A</option>
						<option value="18" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "18") { ?> selected="selected"<?PHP $q = "anpeg3grab"; }} ?>>Anak pegawai ke-3, dst + Grade B</option>
						<option value="19" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "19") { ?> selected="selected"<?PHP $q = "pintodsem2"; }} ?>>Siswa pindahan ke Toddler semester II</option>
						<option value="20" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "20") { ?> selected="selected"<?PHP $q = "pinpgtksem2"; }} ?>>Siswa pindahan ke PG/TK A/TK B Semester II</option>
						<option value="21" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "21") { ?> selected="selected"<?PHP $q = "pinsd34"; }} ?>>Siswa pindahan ke SD 3-4</option>
						<option value="22" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "22") { ?> selected="selected"<?PHP $q = "pinsd56"; }} ?>>Siswa pindahan ke SD 5-6</option>
						<option value="23" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "23") { ?> selected="selected"<?PHP $q = "pinsmp8"; }} ?>>Siswa pindahan ke SMP 8</option>
						<option value="24" <?PHP if(!empty($_POST["disc_cat_adm"])) { if($_POST["disc_cat_adm"] == "24") { ?> selected="selected"<?PHP $q = "pinsmp9"; }} ?>>Siswa pindahan ke SMP 9</option>
						</select> 
                    <?PHP
					} else {
					
						$no_sisda_enc			= mysql_real_escape_string($_POST["no_sisda"]);
						$src_siswa_finance 		= "select discount_payment from siswa_finance where no_sisda = '$no_sisda_enc'";
						$query_siswa_finance	= mysqli_query($mysql_connect, $src_siswa_finance) or die("terjadi kesalahan".mysql_error());
					}
					?>                   
                    </td>
                </tr>
            ...
            <?PHP
				...
				if($jenjang == "Toddler") {
					...
					$jenjang_disc = "pgx";
				} else if($jenjang == "TK A") {
					$jenjang_disc = "tka";
				} else if($jenjang == "TK B") {
					$jenjang_disc = "tkb";
				} else if($jenjang == "SD") {
					$jenjang_disc	= "sdx";
				} else if($jenjang == "SMP") {
				...
				*/
				...
				$src_select_disc_adm	= "select * from set_cat_adm_bi_ma where periode = '$periode_enc' and jenjang = '$jenjang_disc' and set_cat_adm = '$set_cat_adm_enc'";
			...
			$jenjang_enc	= mysql_real_escape_string($jenjang);
			$tingkat_enc	= mysql_real_escape_string($_POST["tingkat"]); 
			...
			/*
			if($jenjang_enc == "Toddler") 	{ $tingkat = "1"; }
			else if($jenjang_enc == "PG") 	{ $tingkat = "1"; }
			else if($jenjang_enc == "TK A")	{ $tingkat = "2"; }
			else if($jenjang_enc == "TK B")	{ $tingkat = "3"; }
			else if($jenjang_enc == "SD") 	{ $tingkat = substr($tingkat_enc,0,1); }
			else if($jenjang_enc == "SMP") 	{ $tingkat = substr($tingkat_enc,0,1); } 
			*/
			...
			//$src_get_spp	= "select * from set_spp where periode = '$periode_enc' and jenjang = '$jenjang_enc' and tingkat = '$tingkat_enc' and ket_disc = '$ket_disc'";
            ...
            <?PHP
				...
				echo "<span id=\"text_normal_black\"><i>Data discount SPP dan KS untuk level $jenjang_enc tahun ajaran $periode_enc belum dibuat. Silahkan hubungi Administrator Keuangan</i></span>";
			}
			?>
            <hr noshade="noshade" color="#666666" size="1" />                  
            </div>  
