...
if($do_check_spp_next_year == "ok") {
	...
	?>	
		<span id="text_normal_black">Prosentase pembayaran untuk SPP tahun ajaran <?= $edu_year_siswa_next ?> belum dilakukan</span><br /><input type="button" value="Setting Persen Pembayaran anak guru / umum" onclick="window.location.href='?pl=persen_bayar';" />
	<?PHP
		} else if($status_check_set_spp == "belom_lengkap") {
	?>	
		<span id="text_normal_black">Nominal biaya SPP, ICT, KS atau e-Learning tahun ajaran <?= $edu_year_siswa_next ?>  belum lengkap / belum ditentukan</span><br /><input type="button" value="Setting SPP tahun <?= $edu_year_siswa_next ?>" onclick="window.location.href='?pl=spp_setting&j=<?PHP echo $src_next_jenjang_spp; //$next_daful_spp dapet dari page_transaction.php ?>';" />
	<?PHP
		} else if($status_check_set_spp == "lengkap") {
	?>	
		<span id="text_normal_black">Lakukan input nominal SPP untuk tahun ajaran <?= $edu_year_siswa_next ?></span><br /><input type="button" value="Setting SPP tahun <?= $edu_year_siswa_next ?>" onclick="window.location.href='engine.php?case=add_spp_next_year&n=<?= $no_sisda_enc; ?>&p=<?= $edu_year_siswa_next; ?>&en=<?= $src_check_data_finance_next_year_eng; ?>&t=<?= $src_next_tingkat_spp; ?>&s=<?= $src_cur_status; ?>';" />
	<?PHP
		}
	}
}
