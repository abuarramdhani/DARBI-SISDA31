...
if($do_check_daftar_ulang == "ok") {
	...
	$src_check_daful_next_year = "
									select nominal,jenjang,cat_adm from set_cat_adm_bi_ma 
									where
									((jenjang = 'tkb' and  cat_adm = 'kegia' and set_cat_adm = 'dafultkb') or
									(jenjang = 'tka' and  cat_adm = 'kegia' and set_cat_adm = 'dafultka') or
									(jenjang = 'tka' and  cat_adm = 'serag' and set_cat_adm = 'dafultka')) and
	...
	$data_daful_tka_serag = "";
	$data_daful_tka_kegia = "";
	
	while($check_daful_next_year = mysqli_fetch_array($query_check_daful_next_year, check_daftar_ulang)) {
	
		$jenjang_daful_next = $check_daful_next_year["jenjang"]; //echo "<h1>".$check_daful_next_year["cat_adm"]."</h1>";
	
		if($jenjang_daful_next == "tka") {
		
			if($check_daful_next_year["cat_adm"] == "kegia") {
			
				//echo "<h1>1</h1>";
			
				if($check_daful_next_year["nominal"] == "" || $check_daful_next_year["nominal"] == 0) { $data_daful_tka_kegia = "kosong"; }
				else { $data_daful_tka_kegia = "ada"; }
			}
			
			if($check_daful_next_year["cat_adm"] == "serag") {
			
				//echo "<h1>2</h1>";
			
				if($check_daful_next_year["nominal"] == "" || $check_daful_next_year["nominal"] == 0) { $data_daful_tka_serag = "kosong"; }
				else { $data_daful_tka_serag = "ada"; }
			}
			
			if($data_daful_tka_kegia == "kosong" || $data_daful_tka_serag == "kosong") { $data_daful_tka = "kosong"; }
	...
	$num_check_spp_daful_next_year		= mysqli_num_rows($query_check_spp_daful_next_year);
	...
	if($num_check_spp_daful_next_year != 3) {
		...
		while($check_spp_daful_next_year = mysqli_fetch_array($query_check_spp_daful_next_year, MYSQLI_ASSOC)) {
	...
	$src_get_persen 	= "select * from persen_bayar where periode = '$edu_year_siswa_next'";
	$query_get_persen	= mysqli_query($mysql_connect, $src_get_persen) or die(mysqli_query($mysql_connect, ));
	$row_get_persen		= mysqli_num_rows($query_get_persen);
	
	if($row_get_persen == 0)  {
?>	
	<span id="text_normal_white">Prosentase pembayaran untuk anak berstatus umum dan anak guru tahuna ajaran <?= $edu_year_siswa_next ?> belum dilakukan</span><br /><input type="button" value="Setting Persen Pembayaran anak guru / umum" onclick="window.location.href='?pl=persen_bayar';" />
<?PHP	
	} else if($data_daful_tka_spp == "ada") {
?>	
