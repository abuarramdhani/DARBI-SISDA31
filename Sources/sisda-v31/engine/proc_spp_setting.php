...
if(isset($_SESSION["id"]) && $_SESSION["privilege"] == "2") {
	...
	if(!empty($periode) and !empty($src_j)) {
		...
		if($num_check_exist != 0) {
			...
			//Look, there is a lot of words of explaination about this function
			//But i'm too lazy to rewrite it in here,
			//Please look at page proc_cat_adm_bi_ma_setting.php. I've explained there
			$src_reset_ai	= "alter table set_spp auto_increment = 1";
			...
			for($i = 0; $i<$data_size; $i++) {
				...
				//the value of $_sppxxx has to follow this pattern...: x-yyyy-zzz 
				//the string that we want to use is x,yyyy and  zzz		
				$spp_explode	= explode("-",$cur_data);
				
				//So we get these variables,.....		
				...
				$nominal	= !empty($spp_explode[3]) ? $spp_explode[3] : 0;
			
				//You are a lucky person,..............
				//because you've found the query for inserting our sucks values here. here we go buddy... :)
				//make sure that periode and level is not empty
