...
if(isset($_SESSION["id"]) && $_SESSION["privilege"] == "2") {
	...
	$src_ttl_tempat			= $_POST["ttl_tempat"];
	...
	if(!empty($gambar)) {
		...
		$new_file_name 	= $nameDate."-".$gambar;
		$path			= "photo/".$new_file_name;
		
		$src_chk_photo_ext		= "select * from siswa where id_siswa = '$no_sisda'";
		...
		if($chk_photo_ext != "") {
			...
			if($delete_file) {
				...
				if($upld_photo) {
					...
					$src_insert_photo_name	= ", photo	= '$new_file_name'";
	...
	$src_insert_data_finance	= "update siswa set 
