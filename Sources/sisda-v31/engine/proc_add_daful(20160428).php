...
if(isset($_SESSION["id"]) && $_SESSION["privilege"] == "2") {
	...
	if($enc == $cek_enc) {
		...
		if($j == "TK A") { $jenjang_persen = $get_persen["daful_tka"]; $src_item_byr = 'tka'; $src_set_cat_adm = "dafultka"; }
		if($j == "TK B") { $jenjang_persen = $get_persen["daful_tkb"]; $src_item_byr = 'tkb'; $src_set_cat_adm = "dafultkb"; }
		...
		if($cur_status == "anak guru") { 
			$persen_daful 		= $jenjang_persen / 100; 
			$persen_spp			= $get_persen["spp"] / 100;
			$persen_ict			= $get_persen["ict"] / 100;
			$persen_elearning	= $get_persen["elearning"] / 100;
		}
		...
		$get_data_daful 	= "select * from set_cat_adm_bi_ma where periode = '$p' and jenjang = '$src_item_byr' and cat_adm = 'kegia' and set_cat_adm = '$src_set_cat_adm'";
		$query_data_daful	= mysqli_query($mysql_connect, $get_data_daful) or die(mysql_error());
		$data_daful			= mysql_fetch_array($query_data_daful);
		
		$nominal_daful = $data_daful["nominal"] * $persen_daful;	
