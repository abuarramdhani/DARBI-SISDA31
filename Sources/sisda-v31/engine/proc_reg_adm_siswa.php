...
if(isset($_SESSION["id"]) && $_SESSION["privilege"] == "2") {
	...
	$kat_status_anak	= htmlspecialchars($_POST["kat_status_anak"]); //echo "kat_status_anak: ".$kat_status_anak."<br>";
	...
	//escape them..........
	$enc_nama_siswa			= mysqli_real_escape_string($mysql_connect, $nama_siswa);
	$enc_nama_ayah			= mysqli_real_escape_string($mysql_connect, $nama_ayah);
	$enc_nama_bunda			= mysqli_real_escape_string($mysql_connect, $nama_bunda);
	$enc_kat_status_anak	= mysqli_real_escape_string($mysql_connect, $kat_status_anak);
	$enc_telp_ayah			= mysqli_real_escape_string($mysql_connect, $telp_ayah);
	$enc_telp_bunda			= mysqli_real_escape_string($mysql_connect, $telp_bunda);
	$enc_nama_sekolah_asal	= mysqli_real_escape_string($mysql_connect, $nama_sekolah_asal);
	$enc_stat_sekolah_asal	= mysqli_real_escape_string($mysql_connect, $stat_sekolah_asal);
	$enc_tanggal_daftar		= mysqli_real_escape_string($mysql_connect, $tanggal_daftar);
	$enc_jenjang			= mysqli_real_escape_string($mysql_connect, $jenjang);
	$enc_tingkat			= mysqli_real_escape_string($mysql_connect, $tingkat);
	$enc_periode			= mysqli_real_escape_string($mysql_connect, $periode);
	$enc_shift_test			= mysqli_real_escape_string($mysql_connect, $shift_test);
	$enc_fase_1_date		= mysqli_real_escape_string($mysql_connect, $fase_1_date);
	$enc_fase_2_date		= mysqli_real_escape_string($mysql_connect, $fase_2_date);
	...
	$enc_disc_cat_adm	= mysqli_real_escape_string($mysql_connect, $disc_cat_adm); 
	$enc_pengembangan	= mysqli_real_escape_string($mysql_connect, $pengembangan); 
	$enc_kegiatan		= mysqli_real_escape_string($mysql_connect, $kegiatan); 
	$enc_peralatan		= mysqli_real_escape_string($mysql_connect, $peralatan); 
	$enc_seragam		= mysqli_real_escape_string($mysql_connect, $seragam); 
	$enc_paket			= mysqli_real_escape_string($mysql_connect, $paket); 
	
	//This value will be used for value in table tunggakan
	$enc_sub_total_bima	= mysqli_real_escape_string($mysql_connect, $sub_total_bima); 
	
	$enc_spp			= mysqli_real_escape_string($mysql_connect, $spp);
	$enc_ict			= mysqli_real_escape_string($mysql_connect, $ict);
	$enc_ler			= mysqli_real_escape_string($mysql_connect, $ler);			
	$enc_ks				= mysqli_real_escape_string($mysql_connect, $ks);
	
	//This value will be used for value in table tunggakan
	$enc_sub_total_spp	= mysqli_real_escape_string($mysql_connect, $sub_total_spp);
	
	$enc_zakat_mal		= mysqli_real_escape_string($mysql_connect, $zakat_mal);
	$enc_zakat_profesi	= mysqli_real_escape_string($mysql_connect, $zakat_profesi);
	$enc_infaq_shodaqoh	= mysqli_real_escape_string($mysql_connect, $infaq_shodaqoh);
	$enc_wakaf			= mysqli_real_escape_string($mysql_connect, $wakaf);
	$enc_lain_lain		= mysqli_real_escape_string($mysql_connect, $lain_lain);
	...
	$no_sisda_row		= mysqli_fetch_array($query_get_no_sisda, MYSQLI_ASSOC);
	...
	if($query_insert_data_finance) {
		...
		if($query_insert_pengembangan) {
			...
			$num_month			= mysqli_num_rows($query_get_month);	
			...
			if($num_month != 0) {
				...
				*/
				...
				for($i=0; $i<=$current_month; $i++) {			
					...
					if($i == 6) { 	
						//echo "current_month:".$current_month." num_month:".	$i."<br>";			
						if($current_month > $i) { $val_december = "4-0"; } else if($current_month == $i) { $val_december = "1-".$enc_sub_total_spp; }
					}
					if($i == 7) { 	
						//echo "current_month:".$current_month." num_month:".	$i."<br>";			
