...
if(isset($_SESSION["id"]) && $_SESSION["privilege"] == "2") {
	...
	///////////////////////////////////////////////
		...
		if($src_siswa != "" && $bulan != "" && $tahun != "") {
			...
			if($bulan == 1) 	{ $bulan_catering = "jan_cataj = 'y-0'"; }
			if($bulan == 2) 	{ $bulan_catering = "feb_cataj = 'y-0'"; }
			if($bulan == 3) 	{ $bulan_catering = "mar_cataj = 'y-0'"; }
			if($bulan == 4) 	{ $bulan_catering = "apr_cataj = 'y-0'"; }
			if($bulan == 5) 	{ $bulan_catering = "may_cataj = 'y-0'"; }
			if($bulan == 6) 	{ $bulan_catering = "jun_cataj = 'y-0'"; }
			if($bulan == 7) 	{ $bulan_catering = "jul_cataj = 'y-0'"; }
			if($bulan == 8) 	{ $bulan_catering = "aug_cataj = 'y-0'"; }
			if($bulan == 9) 	{ $bulan_catering = "sep_cataj = 'y-0'"; }
			if($bulan == 10) 	{ $bulan_catering = "oct_cataj = 'y-0'"; }
			if($bulan == 11) 	{ $bulan_catering = "nov_cataj = 'y-0'"; }
			if($bulan == 12) 	{ $bulan_catering = "dec_cataj = 'y-0'"; }
			...
			for($i=0; $i < $num_siswa; $i++) {
				...
				//example: jul_cataj (in jenis_tunggakan = catering) the value is 2-175000, it means the student has paid their catering payment for july without arrear, it can't be deleted also 
				//example: sep_cataj (in jenis_tunggakan = catering) the value is 3-225000, it means the student has paid their catering payment for september with arrear, it cant be deleted also 
				if($row_arrear_catering["coba"] != 2 && $row_arrear_catering["coba"] != 4 && $row_arrear_catering["coba"] != 5 && $row_arrear_catering["coba"] != 6 && $row_arrear_catering["coba"] != 7) {			
